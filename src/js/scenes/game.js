...
import Heal from '../gameObjects/heal.js';
...
export default class Game extends Phaser.Scene {
  ...
  create() {
    ...
    this.time.addEvent({
      delay: 100,
      callback: this.incrementScoreForDistance,
      callbackScope: this,
      loop: true,
    });
  }

  incrementScoreForDistance() {
    this.updateScore(1);
  }
  ...
  spawnShake() {
    ...
    const FoeDrop = [
      () => new PowerUp(this, x, y),
      () => new Heal(this, x, y),
      () => new Coin(this, x, y)
    ];

    const randomIndex = Phaser.Math.Between(0, FoeDrop.length - 1);

    const shake = FoeDrop[randomIndex]();

    if (shake instanceof Coin) {
      this.coins.add(shake);
    } else if (shake instanceof Heal) {
      this.heals.add(shake);
    } else if (shake instanceof PowerUp) {
  ...
  addScore() {
    this.scoreText = this.add
        .bitmapText(
            ...
            16,
            "wendy",
            String(this.registry.get("score_player")).padStart(6, "0"),
            25
        )
        .setOrigin(0.5)
        .setScrollFactor(0);
  ...
  addLives() {
    this.livesText = this.add
        .bitmapText(
            ...
            16,
            "wendy",
            String(this.registry.get("lives_player")),
            25
        )
        .setOrigin(0.5)
        .setScrollFactor(0);
  ...
  addPlayers() {
    ...
    this.player = new Player(this, this.center_width, this.game.config.height - 70);
  ...
  hitPlayer(player, shot) {
    ...
    if (this.registry.get("lives_player") <= 0) {
      this.gameOver();
      return;
    }
  ...
  updateScore(points = 0) {
    const score = +this.registry.get("score_player") + points;
    this.registry.set("score_player", score);
  ...
  updateLives(count = 0) {
    const lives = +this.registry.get("lives_player") + count;
    this.registry.set('lives_player', lives);
